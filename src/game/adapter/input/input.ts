...
export class Input {
  ...
  npcPhase(coordinate: number[], isNpcItem: string): void {
    ...
    rl.question(`\nNPC : ${isNpcItem} 이/가 있습니다. [사용 / 미사용] \n INPUT :`, answer => {
      const returned = this._npcDomain.useNpcItem(answer, coordinate, isNpcItem);
      console.log('\n');
  ...
  enemyPhase(coordinate: number[], isEnermyStatus: { hp: number[]; exp: number; atk: number }): void {
    ...
    rl.question(`\n 공격하시겠습니까? \n\n[공격 / 도망] \n INPUT :`, answer => {
      const returned = this._enemyDomain.fightEnemy(answer, coordinate, isEnermyStatus);
  ...
  phaseGuide({ returned, roomType }): void {
    if (roomType['type'] === 'plaza') {
      return this.plazaPhase();
    }

    if (roomType['type'] === 'npc') {
      const isNpcItem = this._npcDomain.getNpcItemByCoordinate(returned.location);
